...
function getActionType(action: any): string | undefined {
        ...
        <div className="text-center mb-4">
          ...
          <div className="flex gap-2 justify-center">
            {[0, 1, 2, 3, 4].map((index) => {
              ...
              const { rank, suit } = card ? getCardDisplayValue(card) : { rank: '', suit: '' } as any;
                ...
                <div
                    ...
                    >
                      <Card className={`${
                        ...
                        getPlayerStatusColor(isCurrentPlayer, player.folded as any, (player as any).allIn, hasWon)
                      }`}>
                        <CardContent className="p-2">
                          <div className="flex items-center gap-2 mb-1">
                            <img 
                              src={(player as any).avatar || '/default-bot-avatar.png'} 
                              alt={player.name}
                          ...
                          <div className="flex gap-0.5 mb-1 justify-center">
                            {player.cards && player.cards.length > 0 && player.cards.map((card, cardIndex) => {
                              const { rank, suit } = getCardDisplayValue(card as any);
                              const color = getCardColor(card as any);
                              return (
                          ...
                          <div className="text-center">
                            {(player as any).folded ? (
                              ...
                              <span className="text-xs font-medium">
                                {(() => {
                                  const type = getActionType((aiDecision as any).action);
                                  const amt = getActionAmount((aiDecision as any).action);
                              ...
                              <span className="text-xs text-yellow-400">Bet ${formatChips((player as any).bet)}</span>
                            ) : (
                      ...
                      <div className="bg-background/95 backdrop-blur border rounded-lg shadow-lg max-h-96 overflow-hidden">
                        ...
                        <div className="overflow-y-auto max-h-80 px-4 py-3 space-y-3">
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="text-sm whitespace-pre-wrap">{(aiDecision as any).reasoning}</div>
                          </div>
                          <div className="flex items-center gap-2">
                            ...
                            <div className="flex-1 bg-secondary rounded-full h-2">
                              <div 
                                ...
                                style={{ width: `${(((aiDecision as any).confidence || 0) * 100).toFixed(0)}%` }}
                              />
                            </div>
                            <span className="text-sm font-medium">{(((aiDecision as any).confidence || 0) * 100).toFixed(0)}%</span>
                          </div>
                        </div>
                      </div>
                    </TooltipContent>
