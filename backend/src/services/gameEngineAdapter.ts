...
export class BattleshipEngineAdapter implements GameEngineAdapter {
  processAction(gameState: any, action: any): any {
    // Apply battleship action to game state
    const newState = { ...gameState };
    
    switch (action.type) {
      case 'place_ship':
        // Handle ship placement
        if (!newState.boards) newState.boards = {};
        if (!newState.boards[action.playerId]) {
          newState.boards[action.playerId] = {
            ships: [],
            hits: [],
            misses: [],
            shipsRemaining: 5
        ...
        newState.boards[action.playerId].ships.push({
          id: action.shipId,
      ...
      case 'fire_shot':
        // Handle shot
        const targetBoard = newState.boards?.[action.targetId];
        if (targetBoard) {
          ...
          for (const ship of targetBoard.ships || []) {
            ...
            if (Math.random() > 0.5) { // Placeholder logic
              isHit = true;
              ship.hits++;
              if (ship.hits >= 3) { // Simplified
                ship.isSunk = true;
                targetBoard.shipsRemaining--;
              }
              break;
            }
          }
          
          if (!newState.shots) newState.shots = [];
          newState.shots.push({
            playerId: action.playerId,
          ...
          // Switch turns
          if (newState.currentTurn === action.playerId && newState.players) {
            const currentIndex = newState.players.findIndex((p: any) => p.id === action.playerId);
            newState.currentTurn = newState.players[(currentIndex + 1) % newState.players.length].id;
          }
        }
        break;
      ...
      case 'ready_up':
        ...
        const player = newState.players?.find((p: any) => p.id === action.playerId);
        if (player) {
          player.isReady = true;
          
          // Check if all ready
          if (newState.players.every((p: any) => p.isReady)) {
            newState.phase = 'battle';
            newState.currentTurn = newState.players[0].id;
          }
        }
        break;
    }
  ...
  getValidActions(gameState: any, playerId: string): any[] {
    ...
    if (gameState.phase === 'placement') {
      // Can place ships or ready up
      actions.push({ type: 'ready_up', playerId });
    } else if (gameState.phase === 'battle' && gameState.currentTurn === playerId) {
      // Can fire at any coordinate
      for (let x = 0; x < 10; x++) {
        for (let y = 0; y < 10; y++) {
          actions.push({
            type: 'fire_shot',
  ...
  isGameComplete(gameState: any): boolean {
    // Check if any player has no ships remaining
    if (gameState.boards) {
      for (const board of Object.values(gameState.boards) as any[]) {
  ...
  getWinner(gameState: any): string | null {
    if (!this.isGameComplete(gameState)) return null;
    
    // Find player with ships remaining
    if (gameState.boards && gameState.players) {
      for (const player of gameState.players) {
        const board = gameState.boards[player.id];
        if (board && board.shipsRemaining > 0) {
          return player.id;
        }
      }
    }
  ...
  getCurrentTurn(gameState: any): string | null {
    return gameState.currentTurn || null;
  }

  getPlayerStates(gameState: any): any[] {
    ...
    return gameState.players.map((player: any) => {
      ...
      return {
        ...
        stats: {
          shipsRemaining: board?.shipsRemaining || 5,
          hits: gameState.shots?.filter((s: any) => 
            s.playerId === player.id && s.result === 'hit'
          ).length || 0,
          misses: gameState.shots?.filter((s: any) => 
            s.playerId === player.id && s.result === 'miss'
          ).length || 0
        }
      };
    });
...
export class GameEngineAdapterFactory {
  static create(gameType: string): GameEngineAdapter {
    switch (gameType) {
      ...
      case 'battleship':
        return new BattleshipEngineAdapter();
      default:
