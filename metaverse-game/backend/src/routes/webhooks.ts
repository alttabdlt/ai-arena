...
// Endpoint to get all valid bots with metaverse agents
router.get('/metaverse/valid-bots', async (_req, res) => {
  try {
    // Get all bots that have metaverse agent IDs
    ...
    // Return list of bot IDs and their metaverse agent IDs
    const validBots = bots.map(bot => ({
      aiArenaBotId: bot.id,
    ...
    console.error('Error fetching valid metaverse bots:', error);
    return res.status(500).json({
      error: 'Internal server error',
      message: error.message,
    });
