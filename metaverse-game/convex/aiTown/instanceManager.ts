...
export const findAvailableInstance = query({
  args: {
    zoneType: ZoneType,
  ...
  handler: async (ctx, args) => {
    ...
    if (availableInstances.length === 0) {
      // No available instances, need to create a new one
      return null;
    }
...
export const createWorldInstance = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Create a new world for this instance
    // TODO: This should integrate with the existing world creation logic
