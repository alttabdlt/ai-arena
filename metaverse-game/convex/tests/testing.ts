...
export const wipeAllTables = internalMutation({
  handler: async (ctx) => {
    for (const tableName of Object.keys(schema.tables)) {
      ...
      await ctx.scheduler.runAfter(0, internal.tests.testing.deletePage, { tableName, cursor: null });
    }
  },
});
...
export const deletePage = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!results.isDone) {
      await ctx.scheduler.runAfter(0, internal.tests.testing.deletePage, {
