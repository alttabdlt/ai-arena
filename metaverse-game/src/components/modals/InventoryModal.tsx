...
export default function InventoryModal({ isOpen, onClose, worldId, playerId, aiArenaBotId }: InventoryModalProps) {
  ...
  // Note: Potion consumption mutations exist but with different path
  const consumePotion = useMutation(api.aiTown.potions.consumeEnergyPotion);
  // Note: consumeBuffPotion doesn't exist in the current API
  const consumeBuff = async (args: any) => {
    console.log('Buff potion consumption not yet implemented', args);
  };
